...
function addItem(id) {
  ...
  if (x) {
    ...
    if (x.qty >= p.stock) {
      ...
      return alert(
        `Sorry, only ${p.stock} ${p.name} available.`
      );
...
function editProduct(id) {
  ...
  const adminArt =
    document.getElementById('adminArt');
...
function saveCustomSettings() {
  ...
  fees = {
    ...
    small:
      Math.max(
        0,
        Number(small.value) || 0
      ),

    medium:
      Math.max(
        0,
        Number(medium.value) || 0
      ),

    large:
      Math.max(
        0,
        Number(large.value) || 0
      )
...
function loadSettings() {
  ...
  // so safely stop if elements don't exist.
  if (
    !base ||
    !small ||
    !medium ||
